...
export const BLOCK_TYPES = {
  ...
  hero: {
    ...
    fields: [
      { key: 'src', label: 'Фотография', type: 'image' },
      {
        ...
        label: 'Где текст',
        type: 'select',
        options: [['over', 'Поверх фотографии'], ['under', 'Под фотографией']],
        hint: 'Под фотографией — надёжнее: фотография едет обычной картинкой, её видно сразу и она остаётся в сохранённом письме',
      },
      { key: 'height', label: 'Высота', type: 'slider', min: 140, max: 480, suffix: 'px', hint: 'Только для текста поверх фотографии. Outlook не умеет тянуться по содержимому — если текст не влезет, он вылезет за картинку' },
      ...
      { key: 'bg', label: 'Цвет под фото', type: 'color', hint: 'Поверх фотографии — его увидят те, у кого картинки не загрузились; под фотографией — это цвет полосы с текстом' },
      { key: 'title', label: 'Заголовок', type: 'text' },
      { key: 'font', label: 'Шрифт', type: 'font' },
    ...
    note: 'Текст поверх фотографии держится на фоновой картинке: Outlook не покажет затемнения, а почта у получателя тянет её последней — пока она грузится, на месте баннера стоит цвет под фото. Если это мешает, поставьте текст под фотографией: тогда она едет обычной картинкой.',
  },
  ...
  iconlist: {
    ...
    create: () => ({
      ...
      gap: 14,
      items: [
        { icon: 'stethoscope', title: 'Приём без очереди', text: 'По предварительной записи' },
        { icon: 'test-tubes', title: 'Анализы за сутки', text: 'Результат придёт на почту' },
      ],
      padding: { ...PAD },
    }),
    fields: [
      {
        ...
        itemFields: [
          ...
          { key: 'image', label: 'Своя картинка', type: 'image', hint: 'Встанет вместо иконки' },
          { key: 'emoji', label: 'Эмодзи', type: 'emoji', hint: 'Запасной знак: его покажут вместо иконки там, где картинки выключены' },
        ],
      },
      { key: 'color', label: 'Цвет текста', type: 'color', clearable: true, hint: 'Понадобится, когда блок стоит на тёмной полосе' },
      { key: 'mutedColor', label: 'Цвет пояснений', type: 'color', clearable: true },
