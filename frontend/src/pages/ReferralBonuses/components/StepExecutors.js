      ...
      <div style={{ padding: '0 20px 20px' }}>
        ...
        {pt !== 'normed' && (
          <div className="rb-exec-flat-section">
            ...
            <div className="rb-exec-sep">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--rb-text-secondary)' }}>Анестезиологическое ассистирование:</span>
                {!readOnly && (
                  <button
                    onClick={() => setShowAnesthForm(v => !v)}
                    style={{ background: 'var(--rb-primary)', border: 'none', borderRadius: 6, width: 24, height: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 16, lineHeight: 1, padding: 0, marginRight: 12 }}
              ...
              <AnesthesiologistRulesList
                ...
                readOnly={readOnly}
              />
              <AnesthesiologistRuleAddForm onAdd={handleAddAnesthesiologistRule} saving={saving} readOnly={readOnly} specialties={serviceSpecialties} visible={showAnesthForm} onToggle={() => setShowAnesthForm(v => !v)} />
            </div>
          </div>
        )}
...
function AnesthesiologistRuleAddForm({ onAdd, saving, readOnly, specialties, visible, onToggle }) {
