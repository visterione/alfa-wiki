...
function parseBlock(lines) {
  ...
  // При превышении порога прогрессивной шкалы 1С печатает удержание не одной
  // строкой, а несколькими: "НДФЛ авг. 2026 1,00" и отдельно "НДФЛ с превышения
  // по ставке 15% авг. 2026 59 998,00". Раньше бралось первое совпадение, и в
  // систему попадал только рубль базовой ставки. Ставок со временем может стать
  // больше двух, поэтому складываем все строки удержания, начинающиеся с НДФЛ:
  // нам нужна общая удержанная сумма, а не её разбивка по ставкам.
  // Ограничение [^\n] держит совпадение внутри одной строки таблицы — строки
  // левой и правой колонок листка склеиваются по общей координате Y, и без
  // этого хвост одной строки утащил бы сумму из следующей.
  let ndfl = null;
  const ndflRe = new RegExp('НДФЛ[^\\n]{0,160}?' + MONTH_YEAR_SUM, 'gi');
  let ndflMatch;
  while ((ndflMatch = ndflRe.exec(text)) !== null) {
    const v = parseRuNum(ndflMatch[1]);
    if (v !== null) ndfl = (ndfl === null ? 0 : ndfl) + v;
  }
