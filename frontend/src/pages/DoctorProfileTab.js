...
export default function DoctorProfileTab({ isAdmin }) {
  ...
  const profileUrl = typeof card?.profileUrl === 'string' ? card.profileUrl.trim() : '';
    ...
    <div className="doctor-profile-tab">
      ...
      {error ? <div className="card"><div className="card-body doctor-profile-empty">{error}</div></div> : card && (
        <div className="doctor-card-readonly">
          ...
          <div className="doctor-card-heading"><div><h2>{card.fullName}</h2><div className="doctor-specialty">{card.specialty}</div><div className="doctor-details">{meta.ageRange && <span><Stethoscope size={14}/>{meta.ageRange}</span>}{card.experience && <span><CalendarDays size={14}/>{card.experience}</span>}{meta.internalNumber && <span><Phone size={14}/>вн. {meta.internalNumber}</span>}{meta.mobileNumber && <span><Smartphone size={14}/>{meta.mobileNumber}</span>}{profileUrl && <a href={profileUrl} target="_blank" rel="noreferrer"><ExternalLink size={14}/>Профиль врача</a>}</div></div><div className="doctor-section-tabs"><button className={section === 'info' ? 'active' : ''} onClick={() => setSection('info')} title="Информация"><Info size={18}/></button><button className={section === 'services' ? 'active' : ''} onClick={() => setSection('services')} title="Услуги"><Stethoscope size={18}/></button><button className={section === 'schedule' ? 'active' : ''} onClick={() => setSection('schedule')} title="Расписание"><Clock3 size={18}/></button></div></div>
