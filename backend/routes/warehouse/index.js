...
router.get('/access', authenticate, async (req, res) => {
  try {
    ...
    res.json({
      allowed: true,
      // Признак администратора. Отдаётся отдельно от прав, потому что права
      // раздаются по дереву складских разделов, а администратор — это статус
      // в портале: у него открыто всё и вдобавок то, чего в дереве нет вовсе.
      // Без этого поля мобильный клиент не мог отличить админа от кладовщика и
      // прятал от него инструменты, рассчитанные только на админа.
