...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    res.json({
      ...
      /**
       * Обещание пришлось сократить (ver. 8.47): на вкладке показателей
       * команды переносы и соблюдение срока теперь считаются в том числе по
       * людям. Оставлять здесь строку про «счётчики переносов по конкретным
       * людям» значило бы обещать то, чего модуль больше не делает, — а
       * невыполняемое обещание хуже отсутствующего.
       *
       * Остальное в силе и остаётся границей: время в приложении, активность
       * и «онлайн» модуль не собирает и собирать не будет. Разница в том, что
       * переносы — это след решения о работе, а активность — наблюдение за
       * человеком.
       */
      deliberatelyAbsent: [
        'время в приложении',
