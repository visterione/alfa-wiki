  ...
  try {
    ...
    const events = await CalendarEvent.findAll({
      where: {
        ...
        [Op.and]: [
          // Завершённое событие остаётся в основном календаре перечёркнутым,
          // но точка мини-календаря должна обозначать только то, что ещё
          // требует внимания. В частности, сюда попадают закрытые блоки задач.
          {
            [Op.or]: [
