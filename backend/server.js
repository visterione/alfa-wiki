...
const reviewCollectorRoutes = require('./routes/review-collector');
...
const serviceMatrixRoutes = require('./routes/service-matrix');
...
const callCenterRoutes           = require('./routes/call-center');
...
// Отписка от почтовых рассылок (ver. 8.43). Смонтирована здесь по тем же
// причинам, что контуры выше: отписывается человек без учётной записи, пришедший
// по ссылке из письма, и authenticate на этом маршруте означал бы форму входа
// вместо кнопки «отписаться». Адрес начинается с /api не для красоты, а потому
// что на бою nginx проксирует на бэкенд только /api, /socket.io и /uploads.
app.use('/api/email-optout', require('./routes/emailOptout'));
...
// Файлы анкет раздела «Вакансии» — часть uploads за проверкой доступа. Причина
// та же, что у вложений чата: там лежат сканы диплома, СНИЛС и паспорта, а
...
app.use('/api/review-collector', reviewCollectorRoutes);
