...
/**
 * Миграция публичного API (ver. 6.06) — создаёт api_clients, submissions, api_request_logs.
 *
 * Запуск на сервере:
...
const MIGRATION_FILE = path.join(__dirname, '..', 'migrations', 'ver. 6.06 add-public-api.sql');

// В dev-режиме sequelize сыплет SQL в консоль и забивает вывод скрипта
sequelize.options.logging = false;

const EXPECTED_TABLES = ['api_clients', 'submissions', 'api_request_logs'];
const EXPECTED_INDEXES = [
  'api_clients_key_prefix_uq',
  'submissions_idempotency_uq',
  'submissions_delivery_status_idx',
  'api_request_logs_client_idx'
];
...
async function existingTables() {
  const [rows] = await sequelize.query(`
    ...
    WHERE schemaname = 'public' AND tablename = ANY(ARRAY['api_clients','submissions','api_request_logs'])
  `);
...
async function main() {
  ...
  console.log('');
  console.log('▶ Миграция ver. 6.06 — публичный API');
  console.log('');
  ...
  console.log('✅ Готово');
  console.log('');
  console.log('   Дальше: создать бота и групповой чат, заполнить в .env');
  console.log('   PUBLIC_FORMS_BOT_TOKEN и PUBLIC_FORM_PATIENT_REGISTRATION_CHAT_ID,');
  console.log('   затем pm2 restart alfa-wiki. Подробно — в PUBLIC_API_SETUP.md');
  console.log('');
}
