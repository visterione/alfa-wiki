...
(function () {
  ...
  /** Тот же цвет, но прозрачный: из него сделаны расходящиеся кольца. */
  ...
  function styles(cfg, fg) {
    ...
    return [
      ...
      // Круги на воде. Кнопка стоит в углу чужой страницы, где её никто не
      // ищет: постоянное медленное движение — единственное, что выдаёт её
      // присутствие, пока человек читает.
      //
      // Кольца лежат в отдельной подложке внутри кнопки, а не в её box-shadow:
      // тень занята подъёмом под курсором, а бесконечная анимация перебила бы
      // её насовсем. Полые, поэтому знак под ними виден.
      '.pulse { position: absolute; inset: 0; border-radius: 50%; pointer-events: none; }',
      '.pulse::before, .pulse::after {',
      '  content: ""; position: absolute; inset: 0; border-radius: 50%;',
      '  border: 2.5px solid ' + rgba(cfg.color, 0.7) + ';',
      '  animation: ripple 2.8s cubic-bezier(.2, .55, .3, 1) infinite;',
      '}',
      // Второе кольцо идёт следом, а не рядом: два одновременных выглядят
      // толстой каймой, а не расходящейся волной.
      '.pulse::after { animation-delay: 1.4s; }',
      // Кольцо держит видимость почти до конца пути и гаснет у самого края:
      // при быстром затухании оно исчезало у самой кнопки, и движения было
      // не разглядеть — эффект читался как еле заметный ореол.
      '@keyframes ripple {',
      '  0%   { transform: scale(1); opacity: .85; }',
      '  60%  { opacity: .3; }',
      '  100% { transform: scale(2.05); opacity: 0; }',
      '}',
      // При открытом окошке кольца не нужны: кнопку уже нашли, и рябь под
      // списком каналов только мешает читать.
      '.root.open .pulse { display: none; }',
  ...
  function build(cfg) {
    ...
    var fab = el('button', {
      ...
      'aria-expanded': 'false'
    }, [el('span', { class: 'pulse' }), chatGlyph, closeGlyph]);
