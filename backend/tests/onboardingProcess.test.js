...
// кадров получил бы пустую вкладку файлов при разрешении их смотреть.
test('в срез кадров попадают сканы, а публичные тексты — нет', () => {
  const app = {
    id: 'a', status: 'approved', medCenterId: 'mc', professions: [],
    fullName: 'Иванов Иван', phone: '+79000000000', email: 'i@example.com',
    form: { snils: '123-456', bio: 'текст для сайта', badgeName: 'И. И. Иванов' }
  };
  const files = [{ kind: 'diploma' }, { kind: 'certificate' }, { kind: 'photo' }];

  const hr = projection.project(app, 'hr_check', files);
  assert.equal(hr.form.snils, '123-456');
  assert.equal('bio' in hr.form, false);
  assert.deepEqual(hr.files.map(f => f.kind).sort(), ['certificate', 'diploma']);

  // Маркетологу по бейджу — по-прежнему только фото.
  assert.deepEqual(projection.project(app, 'badge', files).files.map(f => f.kind), ['photo']);
});
...
// Отмеченное врачом — это заявка «умею и готов», а не решение клиники: часть
// позиций сознательно не берут из-за рентабельности или спроса. Жёсткая сверка
// заперла бы шаг навсегда, поэтому расхождение показывается, но не мешает.
test('внесение услуг закрывается при расхождении с МИС, учётка и расписание — нет', () => {
  const services = proc.getStep('services_mis');
  assert.equal(services.verify, 'mis', 'сверка остаётся — она показывает разницу');
  assert.equal(services.blocking, false, 'но шаг не запирает');

  assert.notEqual(proc.getStep('mis_account').blocking, false, 'без учётки дальше идти нельзя');
  assert.notEqual(proc.getStep('schedule').blocking, false, 'расписание без слотов — не расписание');
});
