...
const User = sequelize.define('User', {
  ...
  adminAccess: {
    ...
    defaultValue: {
      ...
      //
      // До 8.34 флага не было: считалось, что «право быть исполнителем» уже
      // выражено самим назначением. Но выбирать исполнителя приходилось из всех
      // сотрудников портала — сотен людей, большинству из которых раздел не
      // виден вовсе, — и назначить можно было того, кто свою задачу никогда не
      // откроет. Она бы молча протухла по сроку.
      vacancies: false,
      ...
      openLineAdmin: false,
      // Почта (ver. 8.58). Флаг открывает только администрирование: заведение
      // ящиков и раздачу доступов. Видит ли человек сам раздел — решает не он,
      // а наличие строки в mail_account_users: доступ к ящику и есть право.
      // Два места, где настраивается одно и то же, неизбежно разошлись бы.
      mail: false,
      ...
      announcements: false,
      // Маркетинг (ver. 8.22) — акции, рекламные площадки и анонсы одним
      // модулем. Здесь, в отличие от соседей, не флаг, а три уровня на вкладку:
      // акции смотрят регистраторы, чтобы отвечать пациенту по телефону, а
      // заводит их один ответственный маркетолог. Одним флагом пришлось бы
      // выбирать между «регистратор не видит акций» и «регистратор заводит
      // скидку в МИС», и второе тем хуже, что удалить её через API нельзя.
      // Уровни: block — вкладки нет, read — только просмотр, edit — правка.
      // Отдельной таблицы, как у зарплаты и склада, здесь не нужно: ключей
      // три, они не зависят ни от медцентра, ни от назначений.
      marketing: {
        promotions: 'block',
        ads: 'block',
        announcements: 'block'
...
// === ПОЧТА (ver. 8.58) ===
// Почтовый клиент для общих ящиков сети. Одиннадцать таблиц — отдельным файлом
// по той же причине, что склад и вакансии.
const {
  models: mailModels,
  associateMail,
} = require('./mail')(sequelize, DataTypes);

associateMail({ User, MedCenter });
