...
//           «показать расхождение, но дать закрыть» — см. услуги.
...
const STEPS = [
  {
    ...
    // есть отметка, и ворота только держали всю цепочку (расписание, сайт,
    // услуги, колл-центр) лишние сутки, ничего не решая. Обязательным шаг
    // остаётся через чек-лист: без кадров заявка не станет «Запущен».
    after: null,
    verify: 'manual',
    slaHours: 16,
  ...
  {
    // Ключ остался прежним, хотя шаг теперь называется иначе: по нему уже
    // расставлены исполнители в onb_assignments и закрыты задачи прошлых
    // заявок. Переименование ключа осиротило бы и то и другое ради строки,
    // которую всё равно видно только в заголовке.
    key: 'website',
    title: 'Реклама',
  ...
  },
  {
    ...
    // Поэтому расхождение показывается бухгалтеру, но закрыть задачу не мешает.
    blocking: false,
    slaHours: 8,
