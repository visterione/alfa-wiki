...
// Срок округляется вверх до этого шага, и это главное свойство токена (ver. 8.10).
//
// ЧТО БЫЛО НЕ ТАК. Токен выдавался с точностью до миллисекунды, то есть на
// каждый запрос — новый. А подставляется он в адрес картинки (?t=…), и адрес
// вместе с ним менялся тоже. Открытая линия перечитывает состояние раз в пять
// минут и каждый раз получала другой токен: у всех вложений в открытой
// переписке менялся src, браузер выбрасывал их из кэша и скачивал заново. Со
// стороны это выглядит так, будто картинки «пропали, а через пару минут снова
// в порядке», — на самом деле они в этот момент грузятся с нуля, все разом.
// Кэшу браузера при таком адресе не помогало ничто: каждая выдача — новый URL.
//
// Теперь конец жизни округляется до шестичасовой границы: пока граница та же,
// один и тот же человек получает один и тот же токен, адрес картинки не
// меняется, и второй раз она берётся из кэша. Округление только вверх, поэтому
// жить токен стал не меньше прежнего — от суток до суток с четвертью.
//
// Предсказуемость границы ничего не ослабляет: токен подписан HMAC, и знание
// того, на какую секунду он истечёт, подделать подпись не помогает.
const TOKEN_BUCKET_MS = 6 * 60 * 60 * 1000;
...
function issueToken(userId, ttlMs = TOKEN_TTL_MS) {
  const exact = Date.now() + ttlMs;

  // Округляем вверх: токен, выданный секундой позже, должен совпасть с
  // предыдущим — иначе адреса картинок расходятся, см. TOKEN_BUCKET_MS.
  //
  // Но только у долгоживущих. Округление добавляет к сроку до шести часов, и
  // для токена, выписанного на минуту, это не «стабильный адрес», а молча
  // продлённый в триста раз доступ. Просивший короткий срок получает ровно его.
  const exp = ttlMs >= TOKEN_BUCKET_MS
    ? Math.ceil(exact / TOKEN_BUCKET_MS) * TOKEN_BUCKET_MS
    : exact;
