...
/**
 * Чьи задачи человек вправе видеть по названию.
 *
 * Это не то же самое, что область видимости загрузки, и разница принципиальная.
 * Загрузка — это часы: сколько у человека занято, чтобы понимать, можно ли ему
 * что-то поручить. Её видят все участники команды, и это нормально. Название
 * задачи — содержание работы, и оно рассказывает то, о чём коллеге знать
 * незачем: над чем человек занят в другом проекте, что ему поручил кто-то
 * третий, что он завёл себе сам.
 *
 * Раньше правило было одно на двоих: участник команды видел все задачи всех
 * своих сокомандников. Личная задача, поставленная одним человеком другому,
 * оказывалась на доске у четверых посторонних.
 *
 * Теперь название задачи видят: сам исполнитель, автор задачи (это проверяется
 * в маршруте) и руководитель команды — но только над её участниками. Другой
 * руководитель той же команды не подчинённый, и его задачи остаются его делом.
 *
 * С ver. 8.42 это правило отвечает только за ЛИЧНЫЕ задачи. У задачи появилась
 * явная привязка к команде, и командную задачу видит весь состав — см.
 * teamIdsForTasks. Здесь по-прежнему нет наблюдателя (viewer): к чужой личной
 * задаче его роль доступа не даёт, он заведён смотреть на часы.
 */
function taskScope(teams, userId) {
  const scope = new Set([userId]);
  for (const team of teams || []) {
    if (!isLead(team, userId)) continue;
    for (const member of team.members || []) {
      if (member.role === ROLES.MEMBER) scope.add(member.userId);
    }
  }
  return [...scope];
}
...
/**
 * Команды, чьи задачи открыты человеку целиком (ver. 8.42).
 *
 * Парная функция к taskScope и вторая половина одного правила. taskScope
 * отвечает на вопрос «чьи задачи мне видны по людям», эта — «задачи каких
 * команд мне видны по привязке». Задача попадает к человеку, если сработало
 * хоть одно из двух.
 *
 * Здесь считаются все три роли, включая наблюдателя: решено, что наблюдатель
 * видит содержимое команды полностью и отличается только тем, что ничего не
 * может изменить. Запрет на изменение обеспечивается не этой функцией, а
 * маршрутами частей: планировать, переносить и закрывать вправе исполнитель,
 * отменять и согласовывать — автор. Наблюдатель не тот и не другой, и ни один
 * из этих маршрутов его не пропустит.
 *
 * Администратора-исключения тут нет. Все команды модуля заводятся скрытыми, а
 * скрытая команда не выдаётся администратору даже списком — делать для её
 * задач отдельную лазейку значило бы отменить это правило с другой стороны.
 */
function teamIdsForTasks(teams, userId) {
  return (teams || [])
