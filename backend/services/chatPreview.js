...
/**
 * Превью ссылок на рабочие чаты: разбор адреса, чтение og-тегов, аватарки.
 *
 * Общая служба: файл не знает ни одной модели онбординга или вакансий. Он умеет
 * одно — по ссылке-приглашению узнать, как чат называется и как выглядит, и
 * сохранить картинку у нас. Какому модулю эти ссылки принадлежат и где они
 * лежат, решает вызывающий.
 *
 * Вынесено из первого поколения онбординга (удалено в ver. 8.39) при появлении
 * второго: копировать триста строк разговора с чужими сайтами, включая защиту
 * от обращений во внутреннюю сеть, было бы худшим из решений.
 *
 * Название и аватарка собираются при настройке, а не в момент отправки письма:
 * страница приглашения может не ответить, и тогда письмо ушло бы с голыми
 * ссылками именно в тот момент, когда его читают. Здесь же скачивается и
 * аватарка — телеграмовский CDN отдаёт её по временному адресу, который через
 * месяц перестанет открываться прямо в отправленном письме.
 */
...
// Подпапка внутри uploads выбрана вне папки с анкетами намеренно: та закрыта
...
// Имя папки осталось от первого поколения намеренно: в базе уже лежат пути к
// скачанным аватаркам, и переименование каталога сделало бы их битыми ради
// красоты имени.
...
async function downloadAvatar(imageUrl) {
  ...
  try {
    ...
    console.warn('[chatPreview] Аватарка не скачалась:', error.message);
    return null;
  }
}
...
async function copyLocalAvatar(relativePath) {
  try {
    ...
    console.warn('[chatPreview] Аватар чата портала не скопировался:', error.message);
    return null;
  }
}
