 ...
 *
 * ── Что придётся подобрать на живом принтере ─────────────────────────────────
 *
 * Куда смотрит первая точка головки и с какого конца этикетка выходит из
 * приёмника, по документации не выводится: в ней нет ни одного рисунка с
 * готовой лентой в руке. Поэтому поворот (90 или 270) и зеркало вынесены в
 * параметры и доходят до экрана настроек принтера в мобилке — на первой же
 * реальной ленте видно, какая пара верна, и менять её потом не придётся.
 */
 ...
 */
async function labelRasterLines(png, { lengthMm = 80, rotate = 90, mirror = false } = {}) {
