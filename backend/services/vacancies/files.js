...
// Отказ читает кандидат, а не сотрудник, поэтому он должен говорить, что
// делать, а не только что не так.
//
// ── Про электронную трудовую книжку (ver. 8.34) ─────────────────────────────
//
// Это единственный документ анкеты, который у половины кандидатов существует
// только в цифровом виде, и разбирались с ним отдельно. Самой «книжки» файлом
// не бывает: записи ведёт СФР по отчёту работодателя (ЕФС-1, подраздел 1.1), а
// человек получает выписку — СТД-СФР на Госуслугах и в личном кабинете СФР либо
// СТД-Р у бывшего работодателя. На Госуслугах она скачивается PDF-ом,
// заверенным усиленной квалифицированной подписью СФР, и этот PDF мы принимаем
// как есть. Никакого архива с сертификатами там нет.
//
// Рядом с PDF предлагается XML — тот же документ машиночитаемо. Его мы не
// принимаем намеренно: читать анкету будет кадровик, а XML он не откроет.
// Поэтому отказ прямо говорит «скачайте PDF» — иначе человек приносит XML,
// получает невнятное «неподходящий тип файла» и пишет в ответ на письмо.
//
// Бумажную книжку присылают фотографиями страниц — для этого в анкете есть тип
// поля «Несколько файлов».
const ACCEPT_HINT = 'Принимаем PDF и фотографии (JPG, PNG, HEIC, WEBP). '
  + 'Выписку из госсервиса — например, из электронной трудовой книжки — скачайте в PDF: XML и архивы не принимаем.';
