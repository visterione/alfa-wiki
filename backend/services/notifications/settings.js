...
// SMS через Fromni убрана в 8.50: SMS в сети осталась одна — прямая, через
// Имобис. Умолчание на неё не ссылается намеренно: счёт у Имобиса свой у
// каждого филиала (ver. 8.25), и подставлять ступень, которая у половины сети
// не настроена, значило бы обещать доставку, которой не будет.
const DEFAULT_CASCADE = ['telegram', 'max', 'imobis:vk'];
 ...
 *
 * ['telegram','imobis:vk','imobis:sms','notify+vk'] →
 *   [{provider:'bot'}, {provider:'imobis', names:['vk','sms']},
 *    {provider:'fromni', names:['notify+vk']}]
 *
 * Пример не выдуман: ровно так каскад и выглядит на время переезда Notify с
 * Fromni на прямой ВК-канал Имобиса (ver. 8.51) — прямая ступень впереди,
 * ступень агрегатора позади неё запасной.
 */
