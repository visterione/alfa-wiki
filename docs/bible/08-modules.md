...
модерацию). Отвечать можно на ПроДокторов, Яндексе, 2ГИС, НаПоправку и
СберЗдоровье; у ДокТу ответ платный и у сети отключён.

С ver. 8.85:

- **Жалоба** из карточки — тем же путём через очередь парсера. Причины
  повторяют кабинет площадки (`complaintConfig` в `platforms.js`): три у
  ПроДокторов, шесть у 2ГИС, у Яндекса только текст. У НаПоправку кнопки
  жалобы нет даже в кабинете.
- **Удалённые с площадки** отзывы: после прохода парсер сообщает, что видел
  (`POST …/places/:id/pass`), и `removal.js` ставит `platformRemovedAt` —
  сразу, если площадка сама сказала «снят» (ПроДокторов `reject`, 2ГИС
  `isHidden`), или после двух пропусков подряд. Раз в сутки проход идёт по
  всей истории, чтобы заметить и старые. В статистике — доля удалённого
  негатива.
- **Удалённый вход** для СберЗдоровья и ДокТу, которые пускают только через
  Яндекс SmartCaptcha: парсер держит страницу входа открытой, её снимок
  приходит на страницу «Площадки», клики по нему уходят обратно
  (задачи `input`).
- **Ссылка «на площадке»** в карточке: у ПроДокторов и НаПоправку — сам отзыв,
  у Яндекса и 2ГИС — отзывы организации.
