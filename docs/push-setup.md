...
После входа в приложение в логах бэкенда при старте должно появиться:
```
🔔 FCM инициализирован (project alfa-wiki)
```
Если вместо этого `FCM не настроен` — не найден service-account JSON.
