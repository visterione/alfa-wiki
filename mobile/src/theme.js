...
export const lightColors = {
  // Primary. Оттенки наведения и подложки — не круглые числа, потому что до
  // ver. 7.84 их считала из акцента getPalette(); значения сохранены как были,
  // чтобы отказ от выбора цвета не перекрасил интерфейс у тех, кто его не менял
  primary: '#007AFF',
  primaryHover: '#006BE0',
  primaryLight: '#E0EDFB',
  secondary: '#5856D6',
...
export const darkColors = {
  primary: '#0A84FF',
  // Наведение в тёмной теме уходит не в белое, а в светлый вариант того же
  // синего: осветлять и без того светлый на тёмном фоне некуда
  primaryHover: '#007AFF',
  primaryLight: '#1C3854',
  secondary: '#5E5CE6',
  ...
  headerGradientStart: '#0756A6',
  headerGradientEnd: '#0A84FF',
};
...
/**
 * Палитра по названию схемы.
 *
 * До ver. 7.84 сюда приходил ещё и выбранный человеком акцент — восемь цветов,
 * из которых пересчитывались primary, градиент шапки и свой пузырь. Выбор убран:
 * пользовались им единицы, а проверять каждый новый экран приходилось во всех
 * восьми. Цвет теперь один, фирменный синий, и лежит прямо в палитрах выше.
 *
...
/**
 * Тень своего пузыря и кнопки отправки — красится основным цветом, а не чёрным.
 ...
 * Кладётся отдельным слоем поверх заливки, а не подмешивается в цвет: слой
 * одинаково ложится и на синюю кнопку, и на цветную капсулу статуса, тогда как
 * подмешивать пришлось бы в каждый цвет отдельно.
 *
 ...
 * цвет приходит из палитры и меняется вместе с темой, а без прозрачности
 * подложка цитаты и кромка вокруг неё выходят плотнее самого текста.
 *
 ...
 *
 * Красится основным цветом портала, а не собственным цветом склада: модуль с
 * чужим оттенком читался бы как чужая программа. Насыщенность нарочно на грани
 * различимости — это фон, а не пятно.
 */
