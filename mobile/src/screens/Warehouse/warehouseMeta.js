...
/**
 * Подпись места одной строкой: у кабинета номер плюс название, у склада — одно
 * название (ver. 7.47). Без второго правила везде, где строку собирают из
 * номера, получалось бы «Каб. Склад».
 *
 * Для строк списка, у которых есть вторая строчка подписи, берут roomHeadText:
 * там название стоит снизу, и в заголовке оно вышло бы дважды.
 */
...
/**
 * Заголовок строки списка: только номер кабинета.
 *
 * Название кабинета в списках стоит отдельной подписью снизу — серой и мельче,
 * — и повторять его в заголовке через тире («Каб. 415 — Архив», а под ним
 * «Архив») незачем: это одно и то же слово дважды подряд.
 *
 * У склада номера нет, и заголовком остаётся название: оно у него единственное.
 */
export const roomHeadText = (room) => {
  if (!room) return '—';
  if (room.isService) return room.name || room.number;
  return `Каб. ${room.number}`;
};

/** Название кабинета для подписи снизу; пусто, когда оно повторяет номер. */
export const roomSubText = (room) => {
  if (!room || room.isService) return '';
  return room.name && room.name !== room.number ? room.name : '';
};
